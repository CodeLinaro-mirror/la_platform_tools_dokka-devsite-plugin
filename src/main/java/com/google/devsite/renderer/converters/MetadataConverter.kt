    ...
    companion object {
        ...
        fun apiSinceMethodSignature(function: DFunction): String {
            ...
            val paramTypes = parameters.joinToString(",") { param ->
                param.type.rewriteKotlinPrimitivesForJava(useQualifiedTypes = true).javaName()
            }
         ...
         */
        private fun Projection.javaName(): String = when (this) {
            ...
            is GenericTypeConstructor -> dri.possiblyAsJava().fullName
            is Nullable -> inner.javaName()
            is DefinitelyNonNullable -> inner.javaName()
            is TypeAliased -> inner.javaName()
            is UnresolvedBound -> name
            is Variance<*> -> inner.javaName()
