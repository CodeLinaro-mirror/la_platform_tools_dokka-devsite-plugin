...
internal class ParameterDocumentableConverter(
    ...
    fun componentForTypeParameter(param: DTypeParameter, isFromJava: Boolean) =
     ...
     * @param context the documentable this type is part of
    ...
    fun componentForProjection(
        ...
        var generics = projection.generics(isJavaSource, sourceSet, context)
    ...
    private fun componentForLambdaProjectionAsKotlin(
        ...
        val lambdaParams =
            lambdaProjections.map {
                componentForLambdaParameter(
                    projection = it!!,
                    isFromJava = false,
                    sourceSet = sourceSet,
                    context = context,
    ...
    fun Projection.generics(
        ...
        when (this) {
            ...
            is Nullable -> this.inner.generics(isJavaSource, sourceSet, context)
            is TypeParameter,
            ...
            is Variance<*> -> this.inner.generics(isJavaSource, sourceSet, context)
            is TypeAliased -> this.inner.generics(isJavaSource, sourceSet, context)
            is DefinitelyNonNullable -> this.inner.generics(isJavaSource, sourceSet, context)
        }
        ...
        when (this) {
            ...
            is PrimitiveJavaType ->
                when (displayLanguage) {
                    ...
                    Language.KOTLIN ->
                        pathProvider.linkForReference(DRI("kotlin", name.capitalize()))
                }
    ...
    internal companion object {
        val kotlinPrimitives =
            listOf("Boolean", "Byte", "Char", "Short", "Int", "Long", "Float", "Double")
        private val javaBoxedPrimitives =
            listOf("Boolean", "Byte", "Character", "Short", "Int", "Long", "Float", "Double")
        ...
        private fun Variance<*>.copy(inner: Bound = this.inner): Projection =
            when (this) {
