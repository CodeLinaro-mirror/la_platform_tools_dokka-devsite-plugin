...
internal fun Documentable.getExpectOrCommonSourceSet() =
    sourceSets.singleOrNull()
        ...
        ?: if (this is DClasslike && this.isAsJavaSynthetic()) sourceSets.first()
        else throw RuntimeException("Unable to determine the expect or common sourceSet for $this")

/** "*Kt" classes from KMP modules get autogenerated copies of sourceSets, all turned into JVM. */
private fun DClasslike.isAsJavaSynthetic() =
    !this.isExpectActual && sourceSets.map { it.displayName }.toSet().single() == "JVM"
