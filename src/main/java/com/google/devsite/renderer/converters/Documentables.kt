...
/** Returns whether the class was synthetically generated by dackka. */
val DClasslike.isSynthetic: Boolean
    get() = !(this as? DClass)?.extra?.allOfType<IsSynthetic>().isNullOrEmpty()

/**
 * An extra property that can be attached to a class to mark it as synthetically constructed by
 * dackka for file facade classes (container for top-level kotlin functions and properties).
 */
internal object IsSynthetic : ExtraProperty<DClass> {
    object PropertyKey : ExtraProperty.Key<DClass, IsSynthetic>

    override val key: ExtraProperty.Key<DClass, *> = PropertyKey
}
