...
import com.google.devsite.renderer.converters.SourceProperty
...
internal class DocumentablesHolder(
        ...
        if (displayLanguage == Language.KOTLIN) {
            packages().forEach { dPackage ->
                dPackage.properties.forEach { property ->
                    property.addToMapping(property.receiver?.type, extensionPropertiesMapping)
                }
            }
        }
     ...
     */
    private fun <T : WithGenerics> T.addToMapping(
        rType: Bound?,
        map: HashMap<DRI, MutableList<T>>,
    ) {
        when (rType) {
            null -> {} // no receiver
            ...
            is TypeParameter -> {
                // Property accessors generated by Dokka don't inherit the property's type
                // parameters,
                // but dackka tracks the originating property in extras.
                val typeParameters =
                    if (this is DFunction) {
                        this.extra[SourceProperty.PropertyKey]?.property?.generics ?: this.generics
                    } else {
                        this.generics
                    }

                typeParameters
                    .find { it.variantTypeParameter.inner.dri == rType.driOrNull }
                    ?.bounds
                    ?.forEach { addToMapping(it, map) }
            }
            ...
            is UnresolvedBound -> {
                ...
                printWarningFor(
                    ...
                    documentableWithError = this as Documentable,
